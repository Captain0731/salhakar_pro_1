      ...
      switch (bookmarkType) {
        case 'judgement':
          ...
          try {
            ...
            // Determine court type from judgment data
            const courtType = completeJudgment.court_name?.toLowerCase().includes('supreme') ||
                              completeJudgment.court?.toLowerCase().includes('supreme')
              ? 'supreme-court'
              : 'high-court';
            
            const judgmentId = completeJudgment.id || completeJudgment.cnr;
            const url = judgmentId ? `/judgment/${courtType}/${judgmentId}` : '/judgment';
            navigate(url, { state: { judgment: completeJudgment, courtType } });
          } catch (err) {
            ...
            if (item.title || item.case_title || item.pdf_link || item.pdf_url) {
              ...
              const courtType = fallbackJudgment.court_name?.toLowerCase().includes('supreme') ||
                                fallbackJudgment.court?.toLowerCase().includes('supreme')
                ? 'supreme-court'
                : 'high-court';
              
              const judgmentId = fallbackJudgment.id || fallbackJudgment.cnr;
              const url = judgmentId ? `/judgment/${courtType}/${judgmentId}` : '/judgment';
              navigate(url, { state: { judgment: fallbackJudgment, courtType } });
            } else {
        ...
        case 'state_act':
          ...
          try {
            ...
            const actId = actData.id || actData.act_id || itemId;
            navigate(`/acts/state/${actId}`, { state: { act: actData, isStateAct: true, section: 'state' } });
          } catch (err) {
            ...
            const fallbackId = item.id || item.act_id || itemId;
            if (fallbackId) {
              navigate(`/acts/state/${fallbackId}`);
            } else {
        ...
        case 'bsa_iea_mapping':
          ...
          try {
            const mappingId = item.id || item.mapping_id || itemId;
            if (mappingId) {
              navigate(`/mapping-details?id=${mappingId}&mapping_type=bsa_iea`);
            } else {
              // Fallback: use state if no ID
              navigate('/mapping-details', { state: { mapping: item } });
            }
          } catch (err) {
            console.error('Error navigating to BSA-IEA mapping:', err);
            const mappingId = item.id || item.mapping_id || itemId;
            if (mappingId) {
              navigate(`/mapping-details?id=${mappingId}&mapping_type=bsa_iea`);
            } else {
        ...
        case 'bns_ipc_mapping':
          ...
          try {
            const mappingId = item.id || item.mapping_id || itemId;
            if (mappingId) {
              navigate(`/mapping-details?id=${mappingId}&mapping_type=bns_ipc`);
            } else {
              // Fallback: use state if no ID
              navigate('/mapping-details', { state: { mapping: item } });
            }
          } catch (err) {
            console.error('Error navigating to BNS-IPC mapping:', err);
            const mappingId = item.id || item.mapping_id || itemId;
            if (mappingId) {
              navigate(`/mapping-details?id=${mappingId}&mapping_type=bns_ipc`);
            } else {
        ...
        case 'bnss_crpc_mapping':
          ...
          try {
            const mappingId = item.id || item.mapping_id || itemId;
            if (mappingId) {
              navigate(`/mapping-details?id=${mappingId}&mapping_type=bnss_crpc`);
            } else {
              // Fallback: use state if no ID
              navigate('/mapping-details', { state: { mapping: item } });
            }
          } catch (err) {
            console.error('Error navigating to BNSS-CrPC mapping:', err);
            const mappingId = item.id || item.mapping_id || itemId;
            if (mappingId) {
              navigate(`/mapping-details?id=${mappingId}&mapping_type=bnss_crpc`);
            } else {
  ...
  return (
    <div className="space-y-2 sm:space-y-6">
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-3 shadow-sm">
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-start flex-1 min-w-0">
              <AlertCircle className="h-4 w-4 text-red-600 mr-3 flex-shrink-0 mt-0.5" />
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-3 sm:p-3 shadow-sm">
        <div className="flex flex-col gap-2 sm:gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2 sm:gap-0">
            ...
            <div>
              <h1 className="text-base sm:text-base md:text-3xl font-bold mb-1" style={{ color: '#1E65AD', fontFamily: "'Bricolage Grotesque', sans-serif" }}>My Bookmarks</h1>
      ...
      {activeTab === 'bookmarks' && (
        <>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-3 sm:p-3 shadow-sm">
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              <select
                ...
                className="px-3 sm:px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors text-xs sm:text-sm"
                style={{ fontFamily: 'Roboto, sans-serif' }}
              >
          ...
          {!currentFolder && (
            <div className="bg-white rounded-lg p-3 sm:p-3 shadow-sm border border-gray-200">
              <div className="flex items-center justify-between mb-4 sm:mb-5">
                <h2 className="text-base sm:text-base font-semibold text-gray-900" style={{ fontFamily: "'Bricolage Grotesque', sans-serif" }}>Folders</h2>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-2">
                {/* New Folder Card */}
                <div
                  onClick={() => setShowCreateFolder(true)}
                  className="relative flex flex-col items-center justify-center p-3 sm:p-3 rounded-lg border-2 border-dashed border-gray-300 hover:border-blue-500 hover:bg-blue-50 transition-all duration-200 group hover:shadow-md cursor-pointer"
                >
                  <div
                ...
                {folders.map((folder) => (
                  <div
                    key={folder.id}
                    className="relative flex flex-col items-center p-3 sm:p-3 rounded-lg border-2 border-dashed border-gray-300 hover:border-blue-500 hover:bg-blue-50 transition-all duration-200 group hover:shadow-md"
                  >
                    <button
                      onClick={() => {
          ...
          {currentFolder && (
            <div className="flex items-center justify-between bg-white rounded-lg p-3 sm:p-3 shadow-sm border border-gray-200">
              <div className="flex items-center min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  <h2 className="text-base sm:text-base font-semibold text-gray-900 mb-0.5 sm:mb-1 truncate" style={{ fontFamily: "'Bricolage Grotesque', sans-serif" }}>{currentFolder.name}</h2>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div className="flex items-center justify-between p-3 sm:p-3 border-b border-gray-200">
              <h2 className="text-base sm:text-base font-semibold truncate flex-1 min-w-0" style={{ color: '#1E65AD', fontFamily: "'Bricolage Grotesque', sans-serif" }}>
              ...
              <div className="flex items-center space-x-1 sm:space-x-1.5 flex-shrink-0">
                <button
                  ...
                  className={`p-1.5 sm:p-1 rounded-lg transition-colors ${viewMode === 'grid' ? 'bg-blue-100' : 'hover:bg-gray-100'
                    }`}
                ...
                <button
                  ...
                  className={`p-1.5 sm:p-1 rounded-lg transition-colors ${viewMode === 'list' ? 'bg-blue-100' : 'hover:bg-gray-100'
                    }`}
            ...
            {loading && bookmarks.length === 0 ? (
              <div className="p-8 sm:p-12 md:p-16 text-center">
                ...
                <h3 className="text-base sm:text-base font-semibold text-gray-900 mb-1.5 sm:mb-2" style={{ fontFamily: "'Bricolage Grotesque', sans-serif" }}>Loading bookmarks...</h3>
              ...
              <div className="p-8 sm:p-12 md:p-16 text-center">
                <div className="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-3 sm:mb-4 rounded-full bg-gray-100 flex items-center justify-center">
                  <Bookmark className="h-4 w-4 sm:h-8 sm:w-8 text-gray-400" />
                </div>
                <h3 className="text-base sm:text-base font-semibold text-gray-900 mb-1.5 sm:mb-2" style={{ fontFamily: "'Bricolage Grotesque', sans-serif" }}>No bookmarks found</h3>
              ...
              <>
                ...
                {selectedItems.length > 0 && (
                  <div className="p-3 sm:p-3 bg-gradient-to-r from-blue-50 to-blue-100 border-b border-blue-200">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0">
                      ...
                      <div className="flex items-center gap-2 w-full sm:w-auto">
                        <button
                          ...
                          className="flex-1 sm:flex-initial px-3 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium shadow-sm"
                          style={{ fontFamily: 'Roboto, sans-serif' }}
                        >
                        ...
                        <button
                          ...
                          className="flex-1 sm:flex-initial px-3 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors font-medium shadow-sm"
                          style={{ fontFamily: 'Roboto, sans-serif' }}
                        >
                          Delete Selected
                        </button>
                ...
                {viewMode === 'grid' ? (
                  <div className="p-3 sm:p-3">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 sm:gap-2">
                      {sortedBookmarks.map((bookmark) => (
                        <div
                          key={bookmark.id}
                          className={`relative bg-white border rounded-lg sm:rounded-lg p-2.5 sm:p-2.5 hover:shadow-xl transition-all duration-200 cursor-pointer group ${selectedItems.includes(bookmark.id) ? 'border-blue-500 bg-blue-50 ring-2 ring-blue-200' : 'border-gray-200 hover:border-blue-200 hover:border-2'
                            }`}
                          onClick={(e) => {
                        ...
                        >
                          ...
                          {/* File Icon */}
                          <div className="flex justify-center mb-2 sm:mb-3 mt-1 sm:mt-1 pl-5 sm:pl-0">
                            <div className={`p-2 sm:p-2 rounded-lg transition-transform group-hover:scale-105 ${bookmark.type === 'judgement' ? 'bg-blue-50' :
                              bookmark.type === 'central_act' || bookmark.type === 'state_act' ? 'bg-green-50' :
                                'bg-purple-50'
                              }`}>
                              <FileText className={`h-4 w-4 sm:h-6 sm:w-6 ${bookmark.type === 'judgement' ? 'text-blue-600' :
                          ...
                          <div className="text-center">
                            <h3 className="font-semibold text-gray-900 text-xs sm:text-sm mb-1.5 sm:mb-2 line-clamp-2 leading-tight min-h-[2rem]" style={{ fontFamily: 'Roboto, sans-serif' }}>
                              {getBookmarkTitle(bookmark)}
                            </h3>

                            {/* Type Badge */}
                            <div className="mb-2 sm:mb-3">
                              <span className={`inline-flex px-2.5 py-1 text-xs font-medium rounded-full whitespace-nowrap ${bookmark.type === 'judgement' ? 'bg-blue-100 text-blue-800' :
                  ...
                  <div className="space-y-2 sm:space-y-2">
                    ...
                    {sortedBookmarks.length > 0 && (
                      <div className="flex items-center gap-2 sm:gap-2 px-2 sm:px-2.5 py-1.5 border-b border-gray-200 bg-gray-50 lg:hidden">
                        <input
                          type="checkbox"
                      ...
                      <table className="w-full">
                        <thead className="bg-gray-50 border-b border-gray-200">
                          <tr>
                            ...
                            <th className="px-2 py-1.5 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider" style={{ fontFamily: 'Roboto, sans-serif' }}>
                              Bookmark
                            </th>
                            <th className="px-2 py-1.5 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider w-32 whitespace-nowrap" style={{ fontFamily: 'Roboto, sans-serif' }}>
                              Type
                            </th>
                            <th className="px-2 py-1.5 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider w-40 whitespace-nowrap" style={{ fontFamily: 'Roboto, sans-serif' }}>
                              Tags
                            </th>
                            <th className="px-2 py-1.5 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider w-32 whitespace-nowrap" style={{ fontFamily: 'Roboto, sans-serif' }}>
                              Date Added
                            ...
                            >
                              ...
                              <td className="px-2 py-2.5">
                                <div className="flex items-center min-w-0">
                                  <div className="flex-shrink-0 mr-3">
                                    <div className={`p-1.5 rounded-lg ${bookmark.type === 'judgement' ? 'bg-blue-50' :
                                      bookmark.type === 'central_act' || bookmark.type === 'state_act' ? 'bg-green-50' :
                                        'bg-purple-50'
                                      }`}>
                                      <FileText className={`h-4 w-4 ${bookmark.type === 'judgement' ? 'text-blue-600' :
                                        bookmark.type === 'central_act' || bookmark.type === 'state_act' ? 'text-green-600' :
                                          'text-purple-600'
                                        }`} />
                                    </div>
                                  ...
                                  <div className="min-w-0 flex-1">
                                    <div
                                      className="text-xs font-semibold text-gray-900 line-clamp-2 mb-1"
                                      style={{ fontFamily: 'Roboto, sans-serif' }}
                                    ...
                                    {(bookmark.item || bookmark).description && (
                                      <div
                                        ...
                                        style={{ fontFamily: 'Roboto, sans-serif' }}
                                        title={(bookmark.item || bookmark).description}
                                      >
                                        {(bookmark.item || bookmark).description}
                                      </div>
                                    )}
                              ...
                              <td className="px-2 py-2.5">
                                <div className="flex flex-wrap gap-1.5">
                                  ...
                                  {(!bookmark.tags || bookmark.tags.length === 0) && (
                                    <span className="text-xs text-gray-400 italic whitespace-nowrap" style={{ fontFamily: 'Roboto, sans-serif' }}>
                                      No tags
                                    </span>
                                  )}
                                </div>
                              ...
                              <td className="px-2 py-2.5 whitespace-nowrap">
                                <div className="flex items-center text-sm text-gray-600" style={{ fontFamily: 'Roboto, sans-serif' }}>
                                  <Clock className="h-3.5 w-3.5 mr-1 text-gray-400 flex-shrink-0" />
                                  <span>{formatDate(bookmark.created_at || bookmark.dateAdded)}</span>
                                </div>
                              </td>
                              <td className="px-2 py-2.5 whitespace-nowrap">
                                <div className="flex items-center space-x-1.5 flex-nowrap">
                                  <button
                                    ...
                                    className={`p-1 rounded-lg transition-colors ${bookmark.is_favorite
                    ...
                    {/* Mobile Card View */}
                    <div className="lg:hidden space-y-2 sm:space-y-2 px-2.5 sm:px-3 pb-2.5 sm:pb-4">
                      {sortedBookmarks.map((bookmark) => {
                        ...
                        return (
                          <div
                            ...
                            className={`p-2.5 sm:p-3 rounded-lg border cursor-pointer transition-all duration-200 hover:shadow-md flex items-start sm:items-center justify-between gap-2 sm:gap-2 ${selectedItems.includes(bookmark.id) ? 'bg-blue-50 border-blue-500' : 'border-gray-200 hover:bg-gray-50'
                              }`}
                          >
                            {/* Selection Checkbox */}
                            <input
            ...
            {pagination.hasMore && !loading && (
              <div className="p-3 sm:p-3 border-t border-gray-200 text-center">
                <button
                  onClick={() => loadBookmarks(pagination.offset + pagination.limit)}
                  className="px-3 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
                >
                  Load More Bookmarks
                </button>
              </div>
      ...
      {showCreateFolder && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50 p-3">
          <div className="bg-white rounded-lg p-3 sm:p-3 w-full max-w-md">
            <h3 className="text-base sm:text-base font-semibold text-gray-900 mb-3 sm:mb-4">Create New Folder</h3>
            <input
              type="text"
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-2 sm:space-x-3 sm:gap-0">
              <button
                ...
                className="px-3 sm:px-3 py-2 text-sm text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 w-full sm:w-auto"
              >
                Cancel
              </button>
              <button
                ...
                className="px-3 sm:px-3 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 w-full sm:w-auto"
              >
                Create
              </button>
            </div>
